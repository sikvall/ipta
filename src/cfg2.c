...
/* This helper function added by Anders "Ichimusai" Sikvall to deal with
   trimming of white space at the beginning and end of strings to be able to
   cope with configuration files and make the following two lines to behave
   as equivalent:

   key=value
   key = value

   Just as well as other varieties, such as indentation and other things in
   the key/value pair that I think is sanity to get rid of.
   ...
   RETURNS
     ...
     The start of the string which should always be the same address as the
     string inserted to trim. This is done in this way because I don't want
     to screw up any malloc()/free() pairs for the strings.
*/
